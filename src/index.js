...
(async () => {
    ...
    await page.waitForSelector('#app > div > div > div.relative.wrapper > div:nth-child(2) > div.table-wrapper > table > tbody > tr.data-row.border-bottom.border-top > td.data.left.sticky-col.first-col');
    // Get the data's latest update date from header
    const dateHeader = await page.evaluate(() => {
        const dateHdr = document.querySelector('#app > div > div > div.relative.wrapper > div:nth-child(2) > div.table-wrapper > table > thead > tr.header-row > th:nth-child(2)');
